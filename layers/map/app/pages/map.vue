    ...
    <div class="map-split">
      ...
      <div class="panel-area">
        <ClientOnly>
          ...
          />
          <div v-else class="flex h-full flex-col items-center justify-center gap-4 p-6 text-center" style="background: rgba(10,15,30,0.96)">
            <div class="flex h-14 w-14 items-center justify-center rounded-full bg-white/5">
              <Icon name="mdi:map-marker-radius-outline" size="28" class="text-brand" />
            </div>
            <div>
              <p class="text-sm font-bold text-white">Near Me</p>
              <p class="mt-1 text-xs text-white/40">Allow location to see stores</p>
            </div>
          </div>
        </ClientOnly>
      </div>
      ...
      <div class="map-area">
        ...
        <ClientOnly>
          ...
          <template #fallback>
            <div class="map-fill flex items-center justify-center bg-[#1a1a2e]">
              <Icon name="mdi:loading" size="32" class="animate-spin text-white/40" />
            </div>
          </template>
        </ClientOnly>
        ...
        <Transition name="fade">
          ...
          >
            <div class="relative flex h-24 w-24 items-center justify-center">
              ...
              <div class="relative z-10 flex h-20 w-20 items-center justify-center rounded-full bg-brand/10 ring-1 ring-brand/30">
                <Icon name="mdi:map-marker-radius-outline" size="40" class="text-brand" />
              </div>
            </div>
            <div>
              <h2 class="text-xl font-black text-white">Discover stores near you</h2>
              <p class="mt-2 max-w-xs text-sm text-white/50">
                Allow location access to find sellers, deals, and products around you.
              </p>
            </div>
            <button
        ...
        <div v-if="locationGranted" class="map-top-ui pointer-events-none">
          <button
            class="pointer-events-auto map-glass-btn"
            aria-label="Back"
            @click="$router.back()"
          >
          ...
          <div class="pointer-events-auto flex flex-1 items-center gap-2 rounded-2xl bg-black/40 px-3.5 py-2.5 backdrop-blur-md border border-white/10 lg:hidden">
            <Icon name="mdi:magnify" size="18" class="text-white/50 shrink-0" />
            <input
            ...
            />
            <button v-if="mobileSearch" class="shrink-0 text-white/40" @click="clearMobileSearch">
          ...
          <button
            class="pointer-events-auto map-glass-btn lg:hidden"
            aria-label="Recenter"
            @click="handleAllowLocation"
          >
        ...
        <div v-if="locationGranted" class="map-pills pointer-events-none lg:hidden">
          <div class="pointer-events-auto space-y-2">
            ...
            />
            <!-- Radius row -->
            <div class="flex items-center gap-2 rounded-2xl bg-black/40 px-3.5 py-2 backdrop-blur-md border border-white/10">
              <Icon name="mdi:map-marker-radius-outline" size="14" class="shrink-0 text-white/40" />
              <input
                type="range"
                :value="radiusKm"
                min="5"
                ...
                @change="onRadiusChange(Number(($event.target as HTMLInputElement).value))"
              />
              <span class="shrink-0 min-w-[44px] text-right text-[11px] font-bold text-white/60">{{ radiusKm }} km</span>
            </div>
          </div>
        </div>

        <!-- ── Bottom sheet (mobile only) ────────────────────────────────── -->
        <div v-if="locationGranted && selectedSeller" class="map-sheet lg:hidden">
          <MapBottomSheet
...
import { useMapSellers, getCachedLocation } from '~~/layers/map/app/composables/useMapSellers'
...
const {
  sellers, loading, error,
  userLat, userLng, activeFilter, radiusKm, categorySlug,
  requestLocation, fetchSellers, fetchPreview,
  setFilter, setSearch, setCategorySlug, setLocation,
} = useMapSellers()
...
// ── Event handlers ────────────────────────────────────────────────────────────
const onSelectSeller = (seller: IMapSeller) => { selectedSeller.value = seller }
...
onMounted(() => {
  ...
  if (cached) {
    ...
    navigator.permissions?.query({ name: 'geolocation' })
      .then((r) => { if (r.state === 'granted') handleAllowLocation() })
      .catch(() => {})
    return
  }
...
<style scoped>
.map-page {
  position: fixed; inset: 0; z-index: 50; overflow: hidden; background: #0f172a;
}
.map-split {
...
.panel-area {
  display: none; width: 340px; flex-shrink: 0;
}
@media (min-width: 1024px) {
  .panel-area { display: flex; flex-direction: column; }
}
.map-fill {
...
.map-glass-btn {
  ...
  border: 1px solid rgba(255,255,255,0.1); transition: background 150ms;
}
.map-glass-btn:hover { background: rgba(0,0,0,0.6); }

/* ── Filter pills area ────────────────────────────────────────────────────────── */
.map-pills {
  position: absolute;
  top: calc(max(env(safe-area-inset-top, 0px), 12px) + 58px);
  left: 0; right: 0; z-index: 20; padding: 0 12px;
}
...
.map-count-badge {
  ...
  border-radius: 9999px; background: rgba(0,0,0,0.55);
...
.mobile-slider::-moz-range-thumb {
  width: 16px; height: 16px; border-radius: 50%;
  background: #F43F5E; border: 2px solid #fff; cursor: pointer;
}
</style>
